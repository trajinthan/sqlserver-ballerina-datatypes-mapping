...
function insertExactNumeric(jdbc:Client jdbcClient) returns sql:Error?{
    ...
    sql:ParameterizedQuery insertQuery =
            `INSERT INTO ExactNumeric (BigInt_col, Numeric_col, Bit_col, 
             SmallInt_col, Decimal_col, SmallMoney_col, Int_col,TinyInt_col, Money_col) VALUES (${big_int}, ${numeric},
...
function insertString(jdbc:Client jdbcClient) returns sql:Error?{
    ...
    sql:ParameterizedQuery insertQuery =
            `INSERT INTO CharacterString (Char_col,Varchar_col, Text_col) VALUES (${char}, ${varchar},
             ${text})`;
...
function insertUnicodeString(jdbc:Client jdbcClient) returns sql:Error?{
    ...
    sql:ParameterizedQuery insertQuery =
            `INSERT INTO UnicodeString (nChar_col,nVarchar_col, nText_col) VALUES (${nchar}, ${nvarchar},
             ${ntext})`;
...
function insertBinaryString(jdbc:Client jdbcClient) returns sql:Error? {
    ...
    sql:ParameterizedQuery insertQuery =
            `INSERT INTO BinaryString (Binary_col,VarBinary_col,Image_col) VALUES (${binary}, ${varbinary},${image}
            )`;
